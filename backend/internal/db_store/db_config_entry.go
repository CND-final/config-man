...
func (s *Store) DeleteConfigEntry(ctx context.Context, configID string, revision model.ConfigRevision, audit model.AuditLog) error {
	return s.withTx(ctx, func(tx *sql.Tx) error {
...
func insertConfigVersionTx(ctx context.Context, tx *sql.Tx, version model.ConfigVersion) error {
	...
	_, err := tx.ExecContext(ctx, `INSERT INTO config_versions (id, config_entry_id, old_value, new_value, changed_by, change_reason, created_at) VALUES ($1,$2,$3,$4,$5,$6,$7)`, version.ID, version.ConfigEntryID, version.OldValue, version.NewValue, version.ChangedBy, version.ChangeReason, version.CreatedAt)
	return err
}
...
func insertConfigVersionSeedTx(ctx context.Context, tx *sql.Tx, version model.ConfigVersion) error {
	...
	_, err := tx.ExecContext(ctx, `INSERT INTO config_versions (id, config_entry_id, old_value, new_value, changed_by, change_reason, created_at) VALUES ($1,$2,$3,$4,$5,$6,$7) ON CONFLICT (id) DO NOTHING`, version.ID, version.ConfigEntryID, version.OldValue, version.NewValue, version.ChangedBy, version.ChangeReason, version.CreatedAt)
	return err
}
...
func (s *Store) ListConfigVersions(ctx context.Context, configID string) ([]model.ConfigVersion, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT id, config_entry_id, old_value, new_value, changed_by, change_reason, created_at FROM config_versions WHERE config_entry_id = $1 ORDER BY created_at DESC`, configID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&version.ID, &version.ConfigEntryID, &oldValue, &version.NewValue, &version.ChangedBy, &version.ChangeReason, &version.CreatedAt); err != nil {
			return nil, err
		}
		if oldValue.Valid {
