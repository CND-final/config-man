...
export function configFileForEntry(entry) {
  return STANDARD_CONFIG_FILES.find((file) => file.matches(entry)) || STANDARD_CONFIG_FILES[0];
}

export function configsForActiveFile(entries, activeFileId) {
  const file = STANDARD_CONFIG_FILES.find((item) => item.id === activeFileId) || STANDARD_CONFIG_FILES[0];
  return entries.filter((entry) => file.matches(entry));
}

export function ensureActiveConfigFile(state) {
  if (!STANDARD_CONFIG_FILES.some((file) => file.id === state.activeConfigFile)) {
...
function isSecurityConfig(entry) {
  const key = normalizedKey(entry);
  return ['security', 'auth', 'jwt', 'oauth', 'cors', 'tls', 'ssl', 'saml', 'session'].some(
    (prefix) => key === prefix || key.startsWith(`${prefix}.`) || key.includes(`.${prefix}.`)
  );
}
